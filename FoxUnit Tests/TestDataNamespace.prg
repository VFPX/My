**********************************************************************
DEFINE CLASS TestDataNamespace as FxuTestCase OF FxuTestCase.prg
**********************************************************************
	#IF .f.
	*
	...
	*
	LOCAL THIS AS TestDataNamespace OF TestDataNamespace.PRG
	#ENDIF
...
*!*		*********************************************************************
	...
	function TestCloseOpenedCursors
		local My as My
		My = newobject('My', 'My.vcx')
		close tables all
		lnUsed1 = aused(laCursors)
		My.Data.OpenCursorSnapshot()
		open database _samples + 'Northwind\Northwind'
		use alphabetical_list_of_products
		My.Data.CloseOpenedCursors()
		lnUsed2 = aused(laCursors)
		This.AssertTrue('CloseOpenedCursors did not close all cursors', ;
			lnUsed2 = lnUsed1)
	endfunc

	function TestCloseAllInstances
		local My as My
		My = newobject('My', 'My.vcx')
		loHelper1 = createobject('Helper')
		loHelper2 = createobject('Helper')
		My.Data.CloseAllInstances('Customers')
		lnSessions = asessions(laSessions)
		for lnI = 1 to lnSessions
			set datasession to laSessions[lnI]
			This.AssertTrue('CloseAllInstances did not close all instances', ;
				not used('Customers'))
		next lnI
	endfunc
	...
	function TestGoTo
		local My as My
		My = newobject('My', 'My.vcx')
		loHelper = createobject('Helper')
		My.Data.GoTo(15, 'Customers', loHelper.DataSessionID)
		lnSession = set('Datasession')
		set datasession to loHelper.DataSessionID
		select Customers
		This.AssertTrue('GoTo did not work', recno() = 15)
...
define class Helper as Session
	function Init
		open database _samples + 'Northwind\Northwind'
		use Customers
	endfunc
